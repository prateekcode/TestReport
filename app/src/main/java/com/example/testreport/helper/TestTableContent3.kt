...
fun testTableContent3(
    ...
    val lastValues = testTableContent2(context, newPatient, Paint(), Canvas())
    for (i in newPatient.sampleType.conditionList.indices) {
        ...
        when (i) {
            2 -> {
                ...
                canvas.drawRect(
                    10F,
                    lastValues.lastHeight + 15f,
                    237F,
                    lastValues.lastHeight + 25f,
                    paint
                )   //Border of condition name
                textPaintPatient.color = ContextCompat.getColor(context, R.color.black)
                textPaintPatient.textAlign = Paint.Align.CENTER
                val middleValueOfX = (237f - 10f) / 1.8f
                Log.d(TAG, "Checking the last value ${lastValues.lastHeight + 21.5f}")
                Log.d(TAG, "Difference between the spaces: ${142f - lastValues.lastHeight + 21.5f} ")
                canvas.drawText(
                    newPatient.sampleType.conditionList[i].conditionName,
                    middleValueOfX,
                    lastValues.lastHeight + 21.5f,
                    textPaintPatient
                )
                ...
                textPaintPatient.isLinearText = true
                textPaintPatient.isSubpixelText = true
                for (parameter in 0..newPatient.sampleType.conditionList[i].conditionTypeList.lastIndex) {
                    Log.d(TAG, "Parameters are $parameter")
                    if (globalY <= 330f) {
                        globalY =
                            lastValues.lastHeight + 24 + (((parameter - (lastValues.lastValueOfI)) * 10) + 10).toFloat()
                        canvas.drawText(
